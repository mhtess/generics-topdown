...
@Gelman2007 found that participants endorsed the generic when the property was inborn but not when the property was acquired through extrinsic means (i.e., an effect of *property origins*), regardless of how many of the category currently had the property (i.e., no effect of *current state*). 
...
As is evident in Figure Figure\ \@ref(fig:predictiveDoblesResults)A, were both effects of property origins  and of current state such that participants were more likely to endorse the generic when it was a property that was in-born ($\beta = `r format_regression_effects(brm.interaction.fit.summary, "originsintrinsic", 3)`$) and when it was a property that was maintained following the drinking event ($\beta = `r format_regression_effects(brm.interaction.fit.summary, "event_outcomemaintained", 3)`$). 
...
The interaction term was not different from 0 ($\beta = `r format_regression_effects(brm.interaction.fit.summary, "originsintrinsic:event_outcomemaintained", 3)`$).
...
The fact that participants were sensitive to the current state when endorsing the generic suggests that their beliefs about the stability of these properties influenced their judgments.
For example, participants may believe that the fact that the property can be lost based on drinking a chemical indicates that the property is relatively unstable in the category; similarly, if there are claws in the environment such that dobles can find them and put them on, perhaps this external cause of the property is a relatively stable external cause. 
As an exploratory analysis, we looked to see if this second main effect of current state was modulated by the kind of property in question (color vs. body part). 
Figure\ \@ref(fig:predictiveDoblesResults)B shows the endorsements split up by the type of property in question.
We see suggestive evidence that the endorsement of a generic for color properties (e.g., "Dobles are green") seems to be less sensitive to the outcome of the event (i.e., dobles losing their color as a result of drinking a chemical).
An intuiging possibility is that participants believe skin color to be a more stable property than that the body part stimuli (e.g., claws, feathers). 
If that is what is leading more participants to endorse the generics about skin color, we would expect to see it reflected in their predictions about future instances of the category.

The fact that we find a second main effect of current state in addition to property origins, while the original only found only a main effect of origins, makes it worth remarking on the differences between our paradigm and the original study by @Gelman2007.
...
This explicit contrast may have led participants to provide different answers in the conditions where the property origins conflicted with the current state.
Another possibility is that the original study was underpowered to detect such a difference: The original sample size was 14; ours is 80. 


```{r predictiveDoblesResults, fig.width=8, fig.cap="Caption", fig.asp=0.4}
...
fig.dobles.endorsement.property <- ggplot(d.origins.filtered.property.summary, aes(x = origins, y = mean, ymin = ci_lower, ymax = ci_upper, 
                                       fill = event_outcome))+
  geom_bar(stat = 'identity', position = position_dodge(bar_width), color = 'black', width = bar_width)+
  scale_fill_manual(values = c("white", "grey45"))+
  geom_errorbar(position = position_dodge(bar_width), width = bar_width/1.5)+
...
cowplot::plot_grid(
  fig.dobles.endorsement, fig.dobles.endorsement.property,
  fig.dobles.prevalence, fig.dobles.prevalence.property,
  nrow = 2,
  labels = c("A", "B","C","D"),
...
As is evident in Figure\ \@ref(fig:predictiveDoblesResults)C, participants predicted with more certainty that the property would be present in future instnaces when the property that was in-born ($\beta = `r format_regression_effects(brm.interaction.prev.fit.summary, "originsintrinsic", 3)`$) and when it was a property that was maintained following the drinking event ($\beta = `r format_regression_effects(brm.interaction.prev.fit.summary, "event_outcomemaintained", 3)`$). 
<!-- The 95% credible intervals indicate that there is at least a 95% probability that the regression coefficients are greater than 0. -->
The interaction term was not different from 0 ($\beta = `r format_regression_effects(brm.interaction.prev.fit.summary, "originsintrinsic:event_outcomemaintained", 3)`$).

Again, as an exploratory analysis, we break the items down by type of property (skins color vs. body parts). 
Figure\ \@ref(fig:predictiveDoblesResults)D shows the parallel pattern of responses, wherein predictions about skin color were less sensitive to the current state than predictions about body parts. 
This 
...
In keeping with our prediction, both distinctive properties ($\beta = `r format_regression_effects(brms.priors.zero_inf_beta_max.summary, "property_typedistinctive")`$) and dangerous properties ($\beta = `r format_regression_effects(brms.priors.zero_inf_beta_max.summary, "property_typedangerous")`$) were expected to be present in fewer categories. 
The fact that such properties are present in fewer categories relaxes their truth conditions, according to our purely probabilistic semantic account. 
...
Dangerous properties are distinctive properties.

As a secondary analysis, we were interested whether dangerousness properties were also more *projectible*, as hypothesized by @Leslie2008.
...
Though the effect of dangerousness leading to higher ratings of projectibility were numerically positive, the credible intervals were too broad to draw a conclusion ($\beta_{dangerousness} = `r format_regression_effects(brms.priors.project.zeroone_beta_max.summary, "property_typedangerous")`$) and similarly for distinctive properties ($\beta_{distinctive} = `r format_regression_effects(brms.priors.project.zeroone_beta_max.summary, "property_typedistinctive")`$).
...
```{r}
load("cached_results/dangerous-priors-filtered.RData") # d.priors.filtered 
