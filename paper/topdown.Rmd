...
This is confirmed by a Bayesian bernoulli regression model with fixed-effects of property type (dangerous vs. distinctive vs. neither), prevalence levels (treated as a numeric variable), and their interaction.
As well, the model included random intercepts and effects of property type, by participant and by item. 




```{r dangerousEndorsements}
load()
...
Having replicated the results of @Cimpian2010, we now test whether or not the prevalence priors are altered in a similar way by the dangerous information. 
