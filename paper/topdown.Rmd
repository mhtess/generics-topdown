...
format_regression_effects <- function(brm_summary, fixed_effect_name, n_digits = 3){
   e1 <- brm.interaction.fit.summary[["fixed"]][[fixed_effect_name, "Estimate"]]
   e_lower <- brm.interaction.fit.summary[["fixed"]][[fixed_effect_name, "l-95% CI"]]
   e_upper <- brm.interaction.fit.summary[["fixed"]][[fixed_effect_name, "u-95% CI"]]
   return(paste(
...
@Gelman2007 found that participants endorsed the generic when the property was inborn but not when the property was acquired through extrinsic means, regardless of how many of the category currently had the property. 
Figure\ \@ref(fig:predictiveDoblesResults) shows the proportion of participants who endorsed the generic under the four experimental conditions.
To evaluate the effects, we built a Bayesian mixed-effects regression model with fixed-effects of property origins, current state, and their interaction with a random intercept by participant and by property type (color vs. body part). 
There were both effects of property origins `r format_regression_effects(brm.interaction.fit.summary, "originsintrinsic", 3)` and of current state  `r format_regression_effects(brm.interaction.fit.summary, "event_outcomemaintained", 3)` that were, with 95% probability, greater than 0. 



```{r predictiveDoblesResults, fig.width=4, fig.cap="Caption"}
load(file = "cached_results/dobles_endorsement_cis.RData") # d.origins.filtered.summary

bar_width = 0.5
ggplot(d.origins.filtered.summary, aes(x = origins, y = mean, ymin = ci_lower, ymax = ci_upper, 
                                       fill = event_outcome))+
...
To test this, we built a Bayesian regression model using the `brms` package in R.
We transformed the raw numerical responses (numbers between 0-1000) into proportions by dividing by 1000.
We use a "zero inflated beta" linking function, because our dependent measure is proportions between 0 and 1 and because pilot testing revealed the response distributions to be highly non-normal, with substantial probability mass at exactly 0 (i.e., 0 species have the property). 
...
\ref{Table X} shows the the 95% credible interval over the estimated effects for the three conditions. 
