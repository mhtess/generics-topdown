...
Generic language (e.g., "Alligators have big teeth") convey generalizations about categories [@Carlson1977] and are thought to play a critical role in learning abstract knowledge about the world [@Gelmanbook].
For the past decade, there has been mounting evidence that meaning of generic statements cannot simply be reduced to probabilistic or statistical statements [@Carlson1995; @Gelman2007; @Leslie2008; @Prasada2000; @Cimpian2010theory; @Cimpian2010]: Generics are judged true even when the predicated property is only rarely present (e.g., "Mosquitos carry malaria" is true despite very few mosquitos actually carrying malaria).
Instead, generics are thought be a direct, linguistic manifestion of conceptual knowledge and thus subject to certain top-down influence [@Prasada; @Leslie]. @Leslie2007 brings our attention to examples like the following:

1. Sharks attack bathers.
2. Rottweilers maul children. 
3. Mosquitos carry the West Nile Virus.

arguing that these generics are reasonable utterances, even though the actual prevalence of the property is quite low (e.g., less than 1% of mosquitos carry the West Nile Virus), because in each case, the property is somehow *striking, alarming,* or *dangerous*:
...
Recently, however, we have shown that generics *can* be reduced to probabilistic statements, albeit ones that are vague and context-dependent: A generic is true if an instance of the category is subjectively likely to have the property, where what counts as *likely* is determined by one's prior knowledge about the property in a set of categories.
For instance, *carrying malaria* is significantly more true of mosquitos than it is of other animals.
This framework was both sufficiently precise to make fine-grained, quantitative predictions that matched human judgments and sufficiently general to be able to explain both generic (e.g., "Mosquitos carry malaria") and habitual language (e.g., "John). 
The information-theoretic communicative model predicts that differences in endorsements based on striking are mediated by corresponding differences in the prevalence prior distributions.^[
...
First, we replicate the findings of @Cimpian2010 (Expt. 4) that information about the dangerousness of a property increases generic endorsement at low prevalence levels.
Then, we run a prevalence elicitation paradigm to see if these differences in endorsements can be traced back to differences in the prevalence prior distributions, using our computational model with the prevalence priors to predict generic endorsement.





<!-- In the literature on generics, it is long argued that generic meaning cannot be reduced to probabilities or quantification  -->
