...
Instead, generics are thought to be a direct, linguistic manifestion of conceptual knowledge and thus subject to certain top-down influence [@Prasada; @Leslie]. @Leslie2007 brings our attention to examples like the following:
...
This framework is sufficiently general to explain the context-sensitivity of *habitual language* (e.g., "John runs"), as well.
One intruiging claim of the information-theoretic communicative model is that differences in endorsements based on top-down, conceptual knowledge (e.g., strikingness) could be mediated by corresponding differences in the relevant prior knowledge recruited about the properties in the form of expectations about prevalence.^[
...
Here, we test whether the impact of strikingness on judgments about generics observed by @Cimpian2010 be reduced to probabilistic expectations about the prevalence of properties.
If this were so, we would expect information about the dangerousness of a property to alter the relevant prior knowledge recruited by a listener to reflect the kind of prior knowledge associated with *distinctive* properties (i.e., properties that are expected to be present in very few categories).
We investigate this by first replicating the findings of @Cimpian2010 Expt. 4, which showed that information about the dangerousness of a property increases generic endorsement at low prevalence levels.
